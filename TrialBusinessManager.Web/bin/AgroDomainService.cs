...
namespace TrialBusinessManager.Web
{
    ...
    public class AgroDomainService : LinqToEntitiesDomainService<IspahaniAgroLTDEntities>
    {
        ...
        public IEnumerable<StockSummaryModel> GetStockSummary(int? RegionId, int? ProductId, string StartDate, string EndDate)
        {
            ...
            {
                DateTime Start = DateTime.ParseExact(StartDate, "dd-MM-yyyy", null);
                DateTime End = DateTime.ParseExact(EndDate, "dd-MM-yyyy", null).AddDays(1);
        ...
        public IEnumerable<AchievementDashboardModel> GetAchievementDashboardData(int? RegionId, int? ProductId, DateTime? StartDate, DateTime? EndDate)
        {
            IspahaniAgroLTDEntities _context = new IspahaniAgroLTDEntities();
            ...
            if (!StartDate.HasValue) StartDate = CurrentSeason.SeasonStartDate;
            if (!EndDate.HasValue) EndDate = CurrentSeason.SeasonEndDate;
            ...
            List<Dealer> DealerList = _context.Dealers.ToList();
            List<RegionTarget> RegionTargetList = _context.RegionTargets.Where(e => e.SeasonId == CurrentSeason.SeasonId).ToList();
            ...
            List<SalesOfficerTarget> SalesOfficerTargetList = _context.SalesOfficerTargets.Where(e => e.SeasonId == CurrentSeason.SeasonId).ToList();
            ...
            foreach (Ledger dl in AllLedger)
            {
                ...
                else
                {
                    ...
                    temp.RegionSalesTargetVollume = (RegionTargetList.Any(e => e.Product.GroupName == temp.ProductGroupName && e.RegionId == temp.RegionId && e.Season.SeasonStartDate >= StartDate && e.Season.SeasonEndDate <= EndDate)) ?
                                                    RegionTargetList.Where(e => e.Product.GroupName == temp.ProductGroupName && e.RegionId == temp.RegionId && e.Season.SeasonStartDate >= StartDate && e.Season.SeasonEndDate <= EndDate).SingleOrDefault().TargetQuantity : 0;

                    temp.SalesOfficerSalesTargetVollume = (SalesOfficerTargetList.Any(e => e.Product.GroupName == temp.ProductGroupName && e.SalesOfficerId == temp.SalesOfficerId && e.Season.SeasonStartDate >= StartDate && e.Season.SeasonEndDate <= EndDate)) ?
                                                          SalesOfficerTargetList.Where(e => e.Product.GroupName == temp.ProductGroupName && e.SalesOfficerId == temp.SalesOfficerId && e.Season.SeasonStartDate >= StartDate && e.Season.SeasonEndDate <= EndDate).SingleOrDefault().TargetQuantity : 0;
        ...
        public IEnumerable<ProductModel> GetProductsForAchievementDashboard()
        {
            ...
            List<Product> ProductList = _context.Products.Where(e => e.ProductName == "DUMMY" && e.ProductStatus == "Active").OrderBy(r=>r.ProductType).ToList();
              ...
              //      foreach (Product product in ProductList)
                //    {
                    ...
                    //    temp.ProductType = product.SubType;
                        ...
                        temp.Commission += ledgers.Where(e => e.IsDebitOrCredit == false && (e.Method == "Commission On Sale" || e.Method == "Commission On Benefit") && e.PartyId == dealer.DealerId).Sum(e => e.CreditAmount);
            ...
            if (StartDate != null && EndDate != null)
            {
                ...
                foreach (Dealer dealer in DealerList)
                {
                    foreach (Product product in ProductList)
                    {
                        ...
                        temp.Commission += ledgers.Where(e => e.IsDebitOrCredit == false && (e.Method == "Commission On Sale" || e.Method == "Commission On Benefit") && e.PartyId == dealer.DealerId && e.Product.GroupName == product.GroupName).Sum(e => e.CreditAmount);
