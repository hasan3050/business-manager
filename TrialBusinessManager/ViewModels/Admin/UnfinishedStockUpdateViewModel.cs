...
{
    public class UnfinishedStockUpdateViewModel : ViewModelBase
    {
        AgroDomainContext _context = new AgroDomainContext();
        BusyWindow MyWindow = new BusyWindow();
        int flag = 0;

        public UnfinishedStockUpdateViewModel()
        {
            Initialize();
        }
        ...
        void Submit()
        {
            ...
            foreach (var item in Infos)
            {
                ...
                if (_context.InventoryProductInfos.Any(c => c.InventoryId.Equals(item.Inventory.InventoryId) && c.ProductId.Equals(item.Product.ProductId) && c.LotId.Equals(item.LotId)))
                {
                    ...
                    ExistingInfo.UnfinishedQuantity += item.UnfinishedQuantity;
                    ExistingInfo.UnitLotCost = item.UnitLotCost;
                }
                else
                {
                ...
                CreateLog(item, item.UnfinishedQuantity, item.Inventory);
        ...
        void CreateLog(InventoryProductInfo Info, Double Quantity, Inventory Inventory)
        {
            ...
            Log.Method = "Unfinished Stock Update";
        ...
        void LoadData()
        {
            ...
            _context.Load(_context.GetProductsQuery().Where(c=>c.ProductName.Equals("DUMMY"))).Completed += (s, e) =>
            {
                flag++;
            ...
            _context.Load(_context.GetInventoryProductInfoesQuery().Where(c=>c.Product.ProductName.Equals("DUMMY"))).Completed += (s, e) =>
            {
                flag++;
                if (flag == 3)
                    MyWindow.Close();
