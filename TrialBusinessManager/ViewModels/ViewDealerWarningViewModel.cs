...
namespace TrialBusinessManager.ViewModels
{
    public class ViewDealerWarningViewModel : ViewModelBase
    {
        ...
        public ViewDealerWarningViewModel()
        {
            ...
            {
                _context.Load(_context.GetDealersQuery().Where(c => c.Region.RegionName.Equals(UserInfo.Region.RegionName) && c.ActivityStatus != "Pending").Where(t=>t.ExpectedDefaulterDate<date)).Completed += (s, e) =>
                {
                    ...
                    DealerModels = new ObservableCollection<ViewDealerModel>(DealerModels.OrderBy(y => y.Dealer.ActivityStatus));
            ...
            {
                _context.Load(_context.GetDealersQuery().Where(t => t.ExpectedDefaulterDate < date)).Completed += (s, e) =>
                {
                    foreach (Dealer x in _context.Dealers)
                    {
                        ViewDealerModel NewDealerModel = new ViewDealerModel();
        ...
        void viewDealerDetail()
        {
            ...
            {
                WarningDetailWindow Window = new WarningDetailWindow(SelectedDealer.Dealer);
                Window.Show();
            }
            else
            {
